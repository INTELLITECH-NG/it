            ...
            <div class="box box-primary">
                        ...
                        <div class="mailbox-controls with-border text-center">
                            <div class="btn-group">
                                ...
                                <a type="button" class="btn btn-default btn-sm" data-toggle="tooltip" data-container="body" title="Reply"
                                   href="<?php echo site_url('admin/mail/forwardMail/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($mail->id.'*'.$mailType))) ?>">
                                    <i class="fa fa-share"></i></a>
                            </div>
                            <!-- /.btn-group -->
                            <a type="button" class="btn btn-default btn-sm" data-toggle="tooltip" title="Print" id="printButton1">
                     ...
                     <?php if(!empty($attachments)):foreach($attachments as $key => $item): ?>
                        <li>
                            ...
                            <div class="mailbox-attachment-info">
                                <a href=" <?php echo site_url('admin/mail/downloadFile/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($mail->id.'*'.$key)))  ?> " class="mailbox-attachment-name"><i class="fa fa-paperclip"></i><?php echo substr($file[1],0,15) ?></a>
                        <span class="mailbox-attachment-size">
                            <?php echo $this->file_type->formatSizeUnits($item->file_size) ?>
                          <a href="<?php echo site_url('admin/mail/downloadFile/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($mail->id.'*'.$key)))  ?>" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
                        </span>
                            </div>
                        </li>
                ...
                <div class="box-footer">
                    <div class="pull-right">
                        <a type="button" class="btn btn-default" href="<?php echo site_url('admin/mail/forwardMail/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($mail->id.'*'.$mailType))) ?>">
                            <i class="fa fa-share"></i><?= lang('replay') ?></a>
                    </div>
                    <button type="submit" class="btn btn-default"><i class="fa fa-trash-o"></i> <?= lang('delete') ?></button>
                    <input type="hidden" name="id[]" value="<?php echo str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($mail->id.'*'.$type)) ?>">
                    <a type="button" id="printButton" class="btn btn-default"><i class="fa fa-print"></i> <?= lang('print') ?></a>
                </div>
                <!-- /.box-footer -->
                <?php echo form_close() ?>
            </div>
